...
router.post("/run", async (req, res) => {
  console.log("  🏃 Starting new agent run");
  try {
...
router.post("/resume/:runId", async (req, res) => {
  console.log(`   Resuming agent run ${req.params.runId}`);
  try {
...
router.get("/events/:runId", (req, res) => {
  const { runId } = req.params;
console.log(`  📡 SSE connection for run ${runId}`);
  // Set SSE headers
  ...
  res.write(
    `data: ${JSON.stringify({ type: "connected", runId, timestamp: new Date().toISOString() })}\n\n`
  );

  const added = runManager.addSSEClient(runId, res);
  if (!added) {
    res.write(
      `data: ${JSON.stringify({ type: "error", message: "Run not found", timestamp: new Date().toISOString() })}\n\n`
    );
    res.end();
    return;
  }
...
router.get("/run/:runId", (req, res) => {
  ...
  console.log(`   Fetching status for run ${run.runId} (status: ${run.status})`);
