...
import type { AgentStatus, AgentError } from "../../types/index.js";
 ...
 */
export function hitlNode(state: HitlState) {
  if (state.mode === "hitl" && !state.approved) {
    console.log("\n  HITL — pausing for human approval...");
    ...
    if (approval && typeof approval === "object" && "approved" in approval) {
      const a = approval as { approved: boolean; feedback?: string };
      console.log(`  → Human ${a.approved ? "approved" : "rejected"} the draft`);
      return {
        approved: a.approved,
        status: "writing" as AgentStatus,
      };
    }
  }
