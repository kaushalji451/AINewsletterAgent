...
export function routerNode(state) {
    ...
    if (state.revisionCount < state.maxRevisionCount) {
        console.log(`  → Critique failed: revising (attempt ${state.revisionCount + 1}/${state.maxRevisionCount})`);
        return "revise";
    }
    // Max revisions reached: force finalize
    console.warn(`  → Max revisions (${state.maxRevisionCount}) reached: force finalizing`);
